...
<div class="container">
<button><a href='/articles/create' class="btn btn-primary"><h1>create an article</h1></a></button>
  ...
  <tbody>
    ...
    <tr>
      ...
      <td>{{$article->image}}</td>
      <td>
        <form action="/articles/{{$article->id}}" method="post" hidden id="deletearticle_{{$article->id}}">
          @csrf
        @method('DELETE')
        </form>
        <button type="submit" class="btn btn-danger d-block-inline" onClick="document.getElementById('deletearticle_{{$article->id}}').submit();">Delete</button>
        
        <a href="/article/{{$article->id}}/edit" class="btn btn-primary d-block-inline">Edit</a>
